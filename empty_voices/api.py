...
class EmptyVoices(commands.Cog):
    ...
    async def validate_category(self, guild: discord.Guild, category: discord.CategoryChannel):
        ...
        # Since we've deleted some things, we need to refresh the cache and check again.
        try:
        ...
        if not empty_public_channels and needed_channels > 0:
            log.debug(f"Need to create {needed_channels} channel(s) in {category.mention}...")
            ...
            for num in available_numbers[:needed_channels]:
                try:
                    ...
                    log.debug(f"Created temp channel: {channel_name} in {category.mention}")
                except discord.Forbidden:
            ...
            if created_count > 0:
                log.debug(f"Successfully created {created_count} temp channel(s) in {category.mention}")
    ...
    async def cleanup(self, ctx):
        ...
        cleanup_msg += f"• Removed {removed_temp_count} orphaned temp channels from config\n"
        cleanup_msg += f"• Removed {removed_watch_count} missing categories from watch list\n"
        cleanup_msg += f"• {len(valid_temp_channels)} temp channels remain\n"
        cleanup_msg += f"• {len(valid_watch_list)} categories still being watched"
    ...
    async def list(self, ctx):
        ...
        if not temp_channels:
            await ctx.send(f"{ctx.author.mention}, No temporary channels currently exist.")
            return
        ...
        for channel_id in temp_channels:
            ...
            if channel:
                ...
                channel_info.append(f"• {channel.mention} ({member_count} members) - {status}")
                valid_channels.append(channel_id)
            else:
                channel_info.append(f"• Unknown Channel (ID: {channel_id}) - ❌ Missing")
    ...
    async def watching(self, ctx):
        ...
        if not watch_list:
            await ctx.send(f"{ctx.author.mention}, No categories are currently being watched.")
            return
        ...
        await ctx.send(f"{ctx.author.mention}, We are watching: {', '.join(category_names)}")
